  ...
  return (
	<div className='flex flex-col gap-1'>
		...
		<div className={`bg-[#1d1d1d] flex justify-between gap-2 items-center rounded-lg transition-all ${focus ? 'border-[2px] ring-2 ring-[#181818]' : ''} border-[1px] border-[#242424] p-2 px-5`}>
			<input type={props.type} value={props.value} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)} name={props.name} placeholder={props.placeholder} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(e)} autoComplete={'off'} className='text-[#959595] placeholder:text-[#4a494c] w-full outline-0 bg-transparent' />
